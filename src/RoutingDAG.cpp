          ...
          if(temp->isTreeNode()) // if we have a tree node
            {
              ...
              os << "\n\t\t<Node index=\"" << treeNodeIndex;
              os << "\" latitude=\"" << centerLati+temp->getX()*latiPerPixel;
              os << "\" longitude=\"" << centerLong+temp->getY()*longPerPixel;
              os << "\" altitude=\"" << ALTITUDE_AT_BASE_PLANE+ALTITUDE_PER_PIXEL*temp->getZ();
              os << "\" alt_unit=\"ft\" ";
              // print the node type
              switch(temp->getNodeType() )
                {
              ...
              for(int j=0; j<temp->getFreeRadiusVecSize(); j++)
                {
                  ...
                  os << "\n\t\t\t\t<Disc index=\"" << j+1;
                  os << "\" radius=\"" << radius*NMILESPERPIXEL;
                  os << "\" units=\"nm\" probability=\"" << prob << "\"/>";
                }
              os << "\n\t\t\t</Operational_Flexibility_Discs>\n\t\t</Node>";		
            }
        }
          ...
          if(edges[i]->isTreeEdge())
            {
              ...
              treeEdgeIndex++;
              os << "\n\t\t<Arc index=\"1" << treeEdgeIndex;
              os << "\" start_node=\"" << tempIndexHead;
              os << "\" end_node=\"" << tempIndexTail;
              os << "\" cost=\"" << edges[i]->getLength()*NMILESPERPIXEL;
              os << "\" cost_unit=\"nm\" RealIndex=\"" << i << "\">";
              ...
              for(int j=0; j<edges[i]->getRNPVecSize(); j++)
                {
                  ...
                  os << "\n\t\t\t\t<RNP_Level RNP=\"" << radius*NMILESPERPIXEL;
                  os << "\" probability=\"" << prob << "\"/>";
                }
              os << "\n\t\t\t</RNP_Levels>";
              ...
              for(int j=0; j<edges[i]->getPathStretchingVecSize(); j++)
                {
                  ...
                  os << "\n\t\t\t\t<Operational_Flexibility_Rectangle index=\"" << j+1;
                  os << "\" width=\"" << radius*NMILESPERPIXEL << "\" units=\"nm\" ";
                  os << "probability=\"" << prob << "\""; 
                  ...
                  os << "\n\t\t\t\t\t\t<Coord index=\"1\" lat=\"" << headLat;
                  os << "\" lon=\"" << headLon << "\"/>";

                  double tailLat =  centerLati+latiPerPixel*edges[i]->getTail()->getX();
                  double tailLon =  centerLong+longPerPixel*edges[i]->getTail()->getY();
                  os << "\n\t\t\t\t\t\t<Coord index=\"2\" lat=\"" << tailLat;
                  os << "\" lon=\"" << tailLon << "\"/>";

                  os << "\n\t\t\t\t\t\t<Coord index=\"3\" lat=\"" << centerLati+latiPerPixel*x1;
                  os << "\" lon=\"" << centerLong+longPerPixel*y1 << "\"/>";

                  os << "\n\t\t\t\t\t\t<Coord index=\"4\" lat=\"" << centerLati+latiPerPixel*x2;
                  os << "\" lon=\"" << centerLong+longPerPixel*y2 << "\"/>";
                  /****************************************************************/
                  os << "\n\t\t\t\t</Rectangle_Coord>";
                }
              ...
              for(int j=0; j<edges[i]->getWiggleRoomVecSize(); j++)
                {
                  ...
                  os << "\n\t\t\t\t<Operational_Flexibility_Rectangle index=\"" << j+1;
                  os << "\" width=\"" << radius*NMILESPERPIXEL;
                  os << "\" units=\"nm\" probability=\"" << prob << "\""; 
          ...
          for(unsigned int i = 0; i<wIter->size(); i++)
            {
              ...
              if( deviationProbability > routingThresh )
                {
                  os << "\t\t<point lat=\"" << lat << "\" lon=\"" << lon << std::flush;
                  os << "\" ensembleId=\"" << ensembleNumber << std::flush;
                  os << "\" ensembleProb=\"" << wIter->getProbability() << std::flush;
                  os << "\"></point>" << std::endl;
                }
            }
        }
