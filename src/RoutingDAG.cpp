...
namespace
{
  unsigned int oneOffset(unsigned int ndx)
  {
    return 100000+ndx;
  }
...
bool RoutingDAG::outputTreeInformation(double centerLati, double centerLong, double latiPerPixel, double longPerPixel, const std::string &startTime, const std::string &endTime, const std::string &outputName, const std::vector<WeatherData> &wDataSets, double routingThresh)
{
  ...
  if( os.is_open() )
    {   /****************************************************************************************************************************/
      ...
      TiXmlDocument doc;
      TiXmlElement rootElement("Routing_Trees");
      rootElement.SetAttribute("start_time",  startTime.c_str() );
      rootElement.SetAttribute("end_time",  endTime.c_str() );
      rootElement.SetAttribute("units", "unix_time_ms");

      TiXmlNode *root( doc.InsertEndChild(rootElement) );
      TiXmlElement *xmlNodes = new TiXmlElement("Nodes");
      ...
      for(unsigned int i=0; i<entries.size()+nodes.size()+fixes.size(); i++)
        {
          ...
          if(temp->isTreeNode()) // if we have a tree node
            {
              // the position in indexToGetNodePointerIndexVec is 
              // the index value of the node, the stored value is 
              // the one used in getNodePointer(int i) function
              indexToGetNodePointerIndexVec.push_back(i);
              treeNodeIndex++;

              TiXmlElement thisNode("Node");
              thisNode.SetAttribute("index",treeNodeIndex);
              thisNode.SetDoubleAttribute("latitude",centerLati+temp->getX()*latiPerPixel);
              thisNode.SetDoubleAttribute("longitude",centerLong+temp->getY()*longPerPixel);
              thisNode.SetDoubleAttribute("altitude",ALTITUDE_AT_BASE_PLANE+ALTITUDE_PER_PIXEL*temp->getZ() );
              thisNode.SetAttribute("alt_unit","ft");
              // print the node type
              std::string nodeType;
              switch(temp->getNodeType() )
                {
              ...
              thisNode.SetAttribute("type",nodeType.c_str() );
              TiXmlElement opFlex("Operational_Flexibility_Discs");
              // print the operational flexity pairs
              for(int j=0; j<temp->getFreeRadiusVecSize(); j++)
                {
                  TiXmlElement thisDisc("Disc");
                  double radius, prob;
                  temp->getFreeRadiusResults(j, &radius, &prob);                    
                  thisDisc.SetAttribute("index",j+1);
                  thisDisc.SetDoubleAttribute("radius",radius*NMILESPERPIXEL);
                  thisDisc.SetAttribute("units","nm");
                  thisDisc.SetDoubleAttribute("probability",prob);
                  opFlex.InsertEndChild(thisDisc);
                }
              thisNode.InsertEndChild(opFlex);
              xmlNodes->InsertEndChild(thisNode);
            }
        }
      // output the information for deviation nodes of edges. 
      // the deviationNodeIndex variable is for edges' deviation nodes
      ...
      for(unsigned int i=0; i<edges.size(); i++)
        {
          if(edges[i]->isTreeEdge())
            {              
              // output each deviation node information
              for(int j = 0; j<edges[i]->getDeviationNodesSize(); j++)
                {
                  TiXmlElement thisDevNode("Node");
                  Node* temp = edges[i]->getDeviationNode(j);
                  deviationNodeIndex++;
                  thisDevNode.SetAttribute("index",deviationNodeIndex);
                  thisDevNode.SetDoubleAttribute("latitude",centerLati+temp->getX()*latiPerPixel);
                  thisDevNode.SetDoubleAttribute("longitude",centerLong+temp->getY()*longPerPixel);
                  thisDevNode.SetDoubleAttribute("altitude",ALTITUDE_AT_BASE_PLANE+ALTITUDE_PER_PIXEL*temp->getZ() );
                  thisDevNode.SetAttribute("alt_unit","ft");
                  thisDevNode.SetAttribute("type","deviation");
                  ...
                  for(int j=0; j<temp->getFreeRadiusVecSize(); j++)
                    {
                      double radius, prob;
                      temp->getFreeRadiusResults(j, &radius, &prob);
                      TiXmlElement thisOpFlex("Disc");
                      thisOpFlex.SetAttribute("index",j+1);
                      thisOpFlex.SetDoubleAttribute("radius",radius*NMILESPERPIXEL);
                      thisOpFlex.SetAttribute("units","nm");
                      thisOpFlex.SetDoubleAttribute("probability",prob);
                      opFlex.InsertEndChild(thisOpFlex);
                    }
                  thisDevNode.InsertEndChild(opFlex);
                  xmlNodes->InsertEndChild(thisDevNode);
                }
            }
        }
      root->InsertEndChild(*xmlNodes);
      delete xmlNodes;
      // now after the nodes, output the edges' information

      // for each edge in the tree, it will have an index value used for outputing
      std::vector<int>        indexToTreeEdgeIndexVec;

      // record all the tree nodes one by one starting from 1
      int treeEdgeIndex = 0;                          
      // the last output index of tree nodes
      deviationNodeIndex = treeNodeIndex;             
      TiXmlElement *xmlArcs = new TiXmlElement("Arcs");
      for(unsigned int i=0; i<edges.size(); i++)
        {
          if(edges[i]->isTreeEdge())
            {
              // get the head node's printed index, in indexToGetNodePointerIndexVec, 
              // where the position in the std::vector is the index-1
              int unifiedIndexHead = getNodePointerIndex(edges[i]->getHead());        // the head's unified index

              // This call is so clear it documents itself...
              int tempIndexHead = find(indexToGetNodePointerIndexVec.begin(), 
                                       indexToGetNodePointerIndexVec.end(), 
                                       unifiedIndexHead)-indexToGetNodePointerIndexVec.begin()+1;

              // the head's unified index
              int unifiedIndexTail = getNodePointerIndex(edges[i]->getTail());

              // ibid
              int tempIndexTail = find(indexToGetNodePointerIndexVec.begin(), 
                                       indexToGetNodePointerIndexVec.end(), 
                                       unifiedIndexTail) - indexToGetNodePointerIndexVec.begin()+1;

              // the position in edges to its printed index
              indexToTreeEdgeIndexVec.push_back(i);                                           
              treeEdgeIndex++;
              TiXmlElement thisArc("Arc");
              thisArc.SetAttribute("index",oneOffset(treeEdgeIndex) );
              thisArc.SetAttribute("start_node=",tempIndexHead);
              thisArc.SetAttribute("end_node",tempIndexTail);
              thisArc.SetDoubleAttribute("cost",edges[i]->getLength()*NMILESPERPIXEL);
              thisArc.SetAttribute("cost_unit","nm");
              thisArc.SetAttribute("RealIndex",i);
              // print rnp operational flexity pairs

              TiXmlElement rnpLevels("RNP_LEVELS");
              for(int j=0; j<edges[i]->getRNPVecSize(); j++)
                {
                  double radius;
                  double prob;
                  edges[i]->getRNPResults(j, &radius, &prob);
                  TiXmlElement thisRnp("RNP_Level");
                  thisRnp.SetDoubleAttribute("RNP",radius*NMILESPERPIXEL);
                  thisRnp.SetDoubleAttribute("probability",prob);
                  rnpLevels.InsertEndChild(thisRnp);
                }
              thisArc.InsertEndChild(rnpLevels);

              TiXmlElement opFlexRects("Operational_Flexibility_Rectangles");
              ...
              for(int j=0; j<edges[i]->getPathStretchingVecSize(); j++)
                {
                  ...
                  TiXmlElement thisOpFlex("Operational_Flexibility_Rectangle");
                  edges[i]->getPathStretchingResults(j, &radius, &prob);
                  thisOpFlex.SetAttribute("index",j+1);
                  thisOpFlex.SetDoubleAttribute("width",radius*NMILESPERPIXEL);
                  thisOpFlex.SetAttribute("units","nm");
                  thisOpFlex.SetDoubleAttribute("probability",prob); 
                  thisOpFlex.SetAttribute("position","right");
                  thisOpFlex.SetAttribute("type","path_stretch");
                    
                  TiXmlElement rects("Rectangle_Coord");
                  rects.SetAttribute("direction","clockwise");
                  /****************************************************************/
                  ...
                  double headLat =  centerLati+latiPerPixel*edges[i]->getHead()->getX();
                  double headLon =  centerLong+longPerPixel*edges[i]->getHead()->getY(); 

                  TiXmlElement coord("Coord");
                  coord.SetAttribute("index",1);
                  coord.SetDoubleAttribute("lat",headLat);
                  coord.SetDoubleAttribute("lon",headLon);
                  rects.InsertEndChild(coord);

                  double tailLat =  centerLati+latiPerPixel*edges[i]->getTail()->getX();
                  double tailLon =  centerLong+longPerPixel*edges[i]->getTail()->getY();

                  coord.SetAttribute("index",2);
                  coord.SetDoubleAttribute("lat",tailLat);
                  coord.SetDoubleAttribute("lon",tailLon);
                  rects.InsertEndChild(coord);


                  coord.SetAttribute("index",3);
                  coord.SetDoubleAttribute("lat",centerLati+latiPerPixel*x1);
                  coord.SetDoubleAttribute("lon", centerLong+longPerPixel*y1);
                  rects.InsertEndChild(coord);

                  coord.SetAttribute("index",4);
                  coord.SetDoubleAttribute("lat",centerLati+latiPerPixel*x2);
                  coord.SetDoubleAttribute("lon", centerLong+longPerPixel*y2);
                  rects.InsertEndChild(coord);
                  thisOpFlex.InsertEndChild(rects);
                  opFlexRects.InsertEndChild(thisOpFlex);
                }
              ...
              for(int j=0; j<edges[i]->getWiggleRoomVecSize(); j++)
                {
                  ...
                  thisOpFlex.SetAttribute("units","nm"); 
                  thisOpFlex.SetDoubleAttribute("probability",prob); 
                  thisOpFlex.SetAttribute("position","left");
                  thisOpFlex.SetAttribute("type","holding");
                  ...
                  coord.SetAttribute("index",3);
                  coord.SetDoubleAttribute("lat", centerLati+latiPerPixel*edges[i]->getTail()->getX());
                  coord.SetDoubleAttribute("lon", centerLong+longPerPixel*edges[i]->getTail()->getY());
                  rects.InsertEndChild(coord);

                  coord.SetAttribute("index",4);
                  coord.SetDoubleAttribute("lat", centerLati+latiPerPixel*edges[i]->getHead()->getX());
                  coord.SetDoubleAttribute("lon", centerLong+longPerPixel*edges[i]->getHead()->getY());
                  rects.InsertEndChild(coord);
              ...
              thisArc.InsertEndChild(opFlexRects);


              TiXmlElement offNomPts("Off_Nominal_Exit_Points");
              for(int j=0; j<edges[i]->getDeviationNodesSize(); j++)
                {
                  
                  deviationNodeIndex++;
                  TiXmlElement thisOnPoint("Off_Nominal_Exit_Point");
                  thisOnPoint.SetAttribute("index",deviationNodeIndex);
                  offNomPts.InsertEndChild(thisOnPoint);
                }
              thisArc.InsertEndChild(offNomPts);
              xmlArcs->InsertEndChild(thisArc);
            }           
        }
      root->InsertEndChild(*xmlArcs);
      delete xmlArcs;
        
      // output tree branch information here. For each tree branch, output its edge lists
      TiXmlElement *xmlBranches = new TiXmlElement("Branches");
      ...
      for(unsigned int i=0; i<entries.size(); i++)
        {
          ...
          Node* temp = entries[i];
          TiXmlElement thisBranch("Branch");
          thisBranch.SetAttribute("index",i );
          thisBranch.SetDoubleAttribute("rnp_equipage",temp->getDrawingRNP()*NMILESPERPIXEL);
          while(temp->getNodeType()!=FIX_NODE)
            {
              ...
              TiXmlElement thisBranchArc("Branch_arc");
              thisBranchArc.SetAttribute("index",oneOffset(printingIndex));
              ...
              thisBranch.InsertEndChild(thisBranchArc);
            }
          xmlBranches->InsertEndChild(thisBranch);
            
        }
      root->InsertEndChild(*xmlBranches);
      delete xmlBranches;
       ...
       *********************/
      os << "\t<Weather>" << std::endl;
      TiXmlElement *xmlWeather = new TiXmlElement("Weather");
          ...
          for(unsigned int i = 0; i<wIter->size(); i++)
            {
              ...
              if( deviationProbability > routingThresh )
                {
                  TiXmlElement weatherPoint("point");
                  weatherPoint.SetDoubleAttribute("lat", lat);
                  weatherPoint.SetDoubleAttribute("lon", lon);
                  weatherPoint.SetAttribute("ensembleId", ensembleNumber);
                  weatherPoint.SetDoubleAttribute("ensembleProb", wIter->getProbability());
                  xmlWeather->InsertEndChild(weatherPoint);
                }
            }
        }
      root->InsertEndChild(*xmlWeather);
      delete xmlWeather;
#endif
...
bool RoutingDAG::generateTree(const std::vector<WeatherData> &wDataSets, std::vector<double> rnp, double effectiveThres, double routingThres)
{
...
// routingThres means that we compute the weighted total probability of the weathercells p1*(0 or 1) + p2*(0 or 1) +..., 
// if the value < routingThres, then it is considered an obstacle 
bool RoutingDAG::routeBranch(Node *start, unsigned int entryIndex, const std::vector<WeatherData> &wDataSets, double rnp, double effectiveThres, double routingThres)
{
...
bool RoutingDAG::testRemainingBranchWhileMerging(Node *start, const std::vector<WeatherData> &wDataSets, double rnp, double effectiveThres, double routingThres)
{
...
bool RoutingDAG::generateTautenedTree(const std::vector<WeatherData> &wDataSets, std::vector<double> rnp, double effectiveThres, double routingThres)
{
      ...
      for(int i=entries.size()-1; i>=0; i--)
        {
          ...
          if(!routeTautenedTreeBranch(entries[i], i, wDataSets, std::max(rnp[i],minWeatherRadius), effectiveThres, routingThres, topMostTendency))    
            {
...
bool RoutingDAG::routeTautenedTreeBranch(Node *start, unsigned int entryIndex, const std::vector<WeatherData> &wDataSets, double rnp, 
...
bool RoutingDAG::testDistanceTooCloseToMergingNodesOnBranch(Node *current, Node *start)
{
  ...
  // was never too close, then return NEVER too close
  return false;
}

/**
   test if a node is too close to the previous merging node on the current branch

   if there is a previous merging node, note that current is on the current branch
...
bool RoutingDAG::testDistanceTooCloseToMergingNodesOnCurrentBranch(Node* toBeDecided, Node *current, unsigned int entryIndex)
{
  ...
  if(minimumDistanceBetweenMergingNodes<=0 || 
     findFeasibleNextEntryNode(entryIndex)==-1 || 
     !onNextBranch(current, entryIndex) )
    {
      return false;
    }
  // the start of the current branch. Entry node cannot be a merging node
  Node* start = entries[entryIndex];          
  // walk up the branch to find the nearest merging node if there is one
  while(current!=start)                                       
    {
      // the previous node on the current branch
      Node* prevNodeOnCurrentBranch = current->getPrevTreeNode();
      // if the previous node is NOT on the next branch, then the current node is a merging node
      if(!onNextBranch(prevNodeOnCurrentBranch, entryIndex))
        {
          break;
        }
      // walk up the current branch
      current = prevNodeOnCurrentBranch;      
    }
...
/*******************************************/
// after the tree is generated, for each node and edge 
//in the tree, calculate their operational flexibilities
void RoutingDAG::generateOperFlexPairs(const std::vector<double> &radii, 
                                       const std::vector<WeatherData> &wDataSets, 
                                       double effectiveThres)
{
  if(status==TREE_NOT_GENERATED)
    {
  ...
  for(unsigned int i=0; i<entries.size(); i++)
    {
      ...
      // if there is no branch coming out of the current entry node
      if(entries[i]->getDrawingRNP()==0)
        {
          continue;
        }
      while(tempNode->getNodeType()!=FIX_NODE)
        {
          ...
          tempNode->clearFreeRadiusVector();
          // insert operational flexibility pairs of the node and its outgoing tree edge
          for(unsigned int j=0; j<radii.size(); j++)
            {
              double tempProb = tempNode->probabilityThatNodeIsClear(radii[j], wDataSets, effectiveThres);
              tempNode->insertFreeRadiusVec(radii[j], tempProb);
              tempProb = tempEdge->sumOfProbOfAllWeatherCloserThanLaneWidthW(radii[j], wDataSets, effectiveThres);
              // insert into the rnp std::vector
              tempEdge->insertOperFlex(radii[j], tempProb, 1);        
              tempProb = tempEdge->testPathStretchWithWeatherDataSet(radii[j], wDataSets, effectiveThres);
              // insert into the path stretching std::vector
              tempEdge->insertOperFlex(radii[j], tempProb, 2);        
              tempProb = tempEdge->testWiggleRoomWithWeatherDataSet(radii[j], wDataSets, effectiveThres);
              // insert into the wiggle room std::vector
              tempEdge->insertOperFlex(radii[j], tempProb, 3);
            }
          /********************************/
          // generate the deviation node for the edge, 
          // each edge has at most 3 deviation nodes, 
          // the minimum interval between nodes is preset to be rnp*2; 
          // Each node has its own operational flexibility pairs
          double edgelength = tempEdge->getLength();
          double currentRNP = tempEdge->getDrawingRNP();
          // generate the nodes at 1/4 position from head to tail
          if(edgelength>=8*currentRNP)
            {
              Node* temp = new Node((tempEdge->getHead()->getX()*3+tempEdge->getTail()->getX())/4, 
                                    (tempEdge->getHead()->getY()*3+tempEdge->getTail()->getY())/4,
                                    (tempEdge->getHead()->getZ()*3+tempEdge->getTail()->getZ())/4);
              // compute the operational flxibility pairs of each generated deviation node
              for(unsigned int j=0; j<radii.size(); j++)
                {
                  temp->insertFreeRadiusVec(radii[j], 
                                            tempNode->probabilityThatNodeIsClear(radii[j], 
                                                                                 wDataSets, 
                                                                                 effectiveThres));
                }
              tempEdge->insertOperFlexDeviationCandidateNode(temp);
            }
          // generate the nodes at 1/2 position from head to tail
          if(edgelength>4*currentRNP)
            {
              Node* temp = new Node((tempEdge->getHead()->getX()+tempEdge->getTail()->getX())/2, 
                                    (tempEdge->getHead()->getY()+tempEdge->getTail()->getY())/2,
          ...
          // generate the nodes at 3/4 position from head to tail
          if(edgelength>=8*currentRNP)                                                                                
            {
              Node* temp = new Node((tempEdge->getHead()->getX()+3*tempEdge->getTail()->getX())/4, 
                                    (tempEdge->getHead()->getY()+3*tempEdge->getTail()->getY())/4,
          ...
          /*********************************/
          // walk to the next step of the tree
          tempNode = tempNode->getOutNode(tempNode->getTreeOutEdgeIndex());
        }
    }
  // finally, compute the operational flexibility of the fixed nodes
  for(unsigned int i=0; i<fixes.size(); i++)
    {
      Node* tempNode = fixes[i];
      for(unsigned int j=0; j<radii.size(); j++)
        {
          tempNode->insertFreeRadiusVec(radii[j], tempNode->probabilityThatNodeIsClear(radii[j], wDataSets, effectiveThres));
        }
    }
}
...
*/
bool RoutingDAG::areAllNodesFarFromWeather( const std::vector<WeatherData> &wDataSets, 
                                            double rad, 
                                            ...
                                            double routingThresh )
{
  ...
  unsigned int totalNodes = entries.size()+nodes.size()+fixes.size(); 
  std::cout << collisionCount << " nodes intersect with the weather data set out of ";
