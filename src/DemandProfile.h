...
public:
	...
	void generateDemandVector(vector<float> &demandRNPs, double startingAngle, double endingAngle, float nmilesPerPixel);
...
private:
	...
	vector<float> rnp;							// the rnp of each aircraft
	string timeStart, timeEnd;					// the simulation time range
	...
	int numDemands;								// the number of demands that are listed in the input file
private:
