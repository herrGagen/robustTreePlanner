...
bool Quadrant::generateDAG(vector<float> rnps, int n, float effectiveThres, float routingThres, const vector<WeatherData*> &wDatas, RoutingDAG* rDAG)
{
	if(!generateEntryAndFixNodes(rnps, n, effectiveThres, routingThres, wDatas, rDAG))
		return false;
...
void Quadrant::generateRoutingDAGInternalNodes(RoutingDAG *rDAG, vector<float> rnps, int n)
{
	...
	{
		numLayers = 1;
		while((liftedoRadius - liftediRadius)/numLayers>minrnp*3)				// compute the number of layers until the interval in between layer is less than maxrnp*3
			numLayers++;
		numLayers--;															// the actual number of layers (was the number of divisions, which is 1 more)
	}
